...
class GroupedConversationSummaries(BaseModel):
    ...
    coverage: Dict[str, int] = Field(default_factory=lambda: {"available": 0, "unavailable": 0})
    ...
    async def _fetch_patient_visit_summaries(self, last_job_exec_time: datetime) -> Dict[UUID, GroupedConversationSummaries]:
        ...
        try:
            ...
            query = select(ConversationSummaries).where(
                ConversationSummaries.updated_at >= last_job_exec_time
            ).order_by(ConversationSummaries.updated_at.desc())
            ...
            for summary in conversation_summaries:
                try:
                    ...
                    group = grouped_summaries_by_user_id.setdefault(summary.user_id,
                        GroupedConversationSummaries(user_id=summary.user_id, summaries=[]))
                    if metadata.get("is_clinical_summary") is not True or metadata.get("validation_status") != "passed" or metadata.get("processing_outcome") in {"unavailable", "no_documents"}:
                        group.coverage["unavailable"] += 1
                        continue
                    summary_model = ConversationSummary.model_validate(summary)

                    # Create simplified summary
                    simplified_conversion_summary = SimplifiedConversationSummary(
                        processing_context={k: metadata[k] for k in ("processing_outcome", "last_refresh_outcome", "pipeline_version") if k in metadata},
                    ...
                    grouped_summaries_by_user_id[summary_model.user_id].coverage["available"] += 1

                except Exception as e:
                    self.logger.error("Summary conversion failed; error_type=%s", type(e).__name__)
                    continue
        ...
        try:
            ...
            for user_id, user_summaries in grouped_summaries.items():
                try:
                    ...
                    combined_text = f"Source coverage (not clinical evidence): {user_summaries.coverage}\n\n" + combined_text
                    # Generate health insights using the chain
                    health_insights = await self.health_insights_chain.generate_health_insights(combined_text)
