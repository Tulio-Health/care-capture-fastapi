    ...
    try:
        for number, page in enumerate(pages, 1):
            response = await model_call(client.chat.completions.create,
                model=model or settings.DOCUMENT_OCR_MODEL, temperature=0, max_tokens=MAX_OCR_OUTPUT_TOKENS,
                response_format={"type": "json_object"},
                messages=[{"role": "system", "content": OCR_POLICY},
                          {"role": "user", "content": [
            ...
            verification = await model_call(client.chat.completions.create,
                model=model or settings.DOCUMENT_OCR_MODEL, temperature=0, max_tokens=min(1024, MAX_OCR_OUTPUT_TOKENS),
                response_format={"type": "json_object"},
            ...
            if parsed.text.strip():
                ...
                text = DocumentTextExtractor().extract_text(parsed.text.encode(), "text/plain;charset=utf-8")
